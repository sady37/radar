...
import type { RadarObject, ObjectProperties, BoundarySettings, Point } from '../stores/types'
...
const drawCoordinateSystem = (ctx: CanvasRenderingContext2D) => {
	...
	if (canvasStore.showGrid) {
		...
		// X轴网格
		for (let x = originX; x <= 620; x += gridSize) {  // 右侧
			ctx.beginPath()
			ctx.moveTo(x, 0)
			ctx.lineTo(x, 520)
			ctx.stroke()
		}
		for (let x = originX; x >= 0; x -= gridSize) {    // 左侧
			ctx.beginPath()
			ctx.moveTo(x, 0)
			ctx.lineTo(x, 520)
			ctx.stroke()
		...
		// Y轴网格（从顶部向下）
		for (let y = originY; y <= 520; y += gridSize) {
			ctx.beginPath()
			ctx.moveTo(0, y)
			ctx.lineTo(620, y)
			ctx.stroke()
	...
	if (canvasStore.showScale) {
		...
		for (let x = -300; x <= 300; x += 100) {
			...
			if (x !== 0) {  // 不是原点时显示正负号
				...
				ctx.fillText(x > 0 ? `+${x}` : `${x}`, xPos, 5)
				ctx.textBaseline = 'bottom'
				ctx.fillText(x > 0 ? `+${x}` : `${x}`, xPos, 515)
			}
		}
		...
		for (let y = 0; y <= 520; y += 50) {
			const yPos = y * scale.value  // 直接使用y值，不需要转换
			const label = y.toString()

			ctx.textAlign = 'right'
			ctx.textBaseline = 'middle'
			ctx.fillText(label, 20, yPos)     // 左侧刻度
			ctx.textAlign = 'left'
			ctx.fillText(label, 600, yPos)    // 右侧刻度
		}
	...
	const isPointInObject = (x: number, y: number, obj: RadarObject): boolean => {
		...
		if (obj.type === 'Radar' || (obj.type === 'Other' && obj.properties.borderOnly)) {
			return false
		}
	...
	const handleCanvasClick = (event: MouseEvent) => {
		...
		const x = Math.round((event.clientX - rect.left - 310) / scale.value)
		const y = Math.round((event.clientY - rect.top - 260) / scale.value)
	...
	const drawObject = (ctx: CanvasRenderingContext2D, obj: RadarObject) => {
		...
		ctx.save()
		// 现有的位置和旋转变换保持不变...
		ctx.translate(310 + obj.position.x * scale.value, 260 + obj.position.y * scale.value)
		...
		if (obj.id === objectsStore.selectedId) {
			...
			if (obj.type === 'Radar') {
				...
				ctx.strokeRect(-halfLength, -halfWidth,
	...
	const updateMousePosition = (event: MouseEvent) => {
		...
		const x = Math.round((event.clientX - rect.left - 310) / scale.value)  // 到中心点的X偏移
