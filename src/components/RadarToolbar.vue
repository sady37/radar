...
<template>
	<div class="radar-toolbar">
	  ...
	  <div class="property-area">
		...
		<div class="size-row" v-if="['Door', 'Bed', 'Exclude', 'Other'].includes(selectedType)">
...
const validateBoundary = () => {
  if (properties.mode === 'wall') {
    properties.boundary.frontY = Math.min(400, Math.max(30, Math.round(properties.boundary.frontY / 10) * 10))
    properties.boundary.rearY = 0  // wall模式rear固定为0
  } else {  // ceiling模式
    properties.boundary.frontY = Math.min(200, Math.max(10, Math.round(properties.boundary.frontY / 10) * 10))
    properties.boundary.rearY = Math.min(200, Math.max(10, Math.round(properties.boundary.rearY / 10) * 10))
  }

  // left和right的验证
  properties.boundary.leftX = Math.min(300, Math.max(10, Math.round(properties.boundary.leftX / 10) * 10))
  properties.boundary.rightX = Math.min(300, Math.max(10, Math.round(properties.boundary.rightX / 10) * 10))
}



watch(() => objectsStore.selectedId, (newId) => {
 if (newId) {
...
const createObject = () => {
 const objectData = {
   ...
   position: { 
     ...
     y: objectsStore.objects.length * 20
   },
   size: { 
     length: properties.length,
     width: properties.width 
  ...
  .template-area {
    .template-buttons {
      ...
      .template-btn {
        ...
        &.door { background: #a0eda0; }
        &.bed { background: #add8e6; }
        &.exclude { background: #f0e68c; }
        &.other { background: #d3d3d3; }
        ...
        .radar-icon {
          ...
          .radar-circle {
            ...
            height: 100%;
            border-radius: 50%;
            background: linear-gradient(to bottom, #fff 0%, #0cade3 50%, #fff 100%);
          }
