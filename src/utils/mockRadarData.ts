	...
	generateMockTrackData(): PersonData[] {
	  ...
	  if (this.currentPosture === null || 
		...
		if (this.currentPosture === PersonPosture.FallConfirm || 
			this.currentPosture === PersonPosture.Lying||
			this.currentPosture === PersonPosture.SitGroundConfirm)
			 {
		  this.postureDuration = 20;     // 跌倒和躺卧持续20秒
		 } else {
		  this.postureDuration = Math.floor(Math.random() * 3 + 0);  // 其他姿态  0-2秒随机
		 }
		 //this.bedPostureDuration = 30;  // 床上姿态固定20秒
	  }
